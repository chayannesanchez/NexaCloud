...
function cognito_getIdToken() {
    return sessionStorage.getItem('cognito_token');
}

function cognito_getAccessToken() {
    return sessionStorage.getItem('cognito_access_token');
}

function cognito_requireAuth(redirectTo = '../login/login.html') {
    if (!cognito_isAuthenticated()) {
...
function cognito_getSessionSummary() {
    return {
        user: cognito_getCurrentUser(),
        idToken: cognito_getIdToken(),
        accessToken: cognito_getAccessToken(),
        expiresAt: Number(sessionStorage.getItem('cognito_expires_at') || 0)
    };
}
